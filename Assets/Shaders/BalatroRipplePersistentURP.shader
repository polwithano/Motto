Shader "Custom/BalatroRipplePersistentURP"
{
    Properties
    {
        /* ================= BALATRO ================= */
        ...
        _DispTex ("Displacement Field (RG)", 2D) = "black" {}
        _PaintWarp ("Paint Warp Strength", Range(0,2)) = 1.0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (Varyings IN) : SV_Target
            {
                ...
                // Persistent displacement field (paint memory)
                float4 dispTex = SAMPLE_TEXTURE2D(_DispTex, sampler_DispTex, IN.uv);
                float2 disp = DecodeDisp(dispTex);

                // Use it as paintOffset (scale is artistic)
                float2 paintOffset = disp * _PaintWarp;

                return BalatroEffect(resolution, fragCoord, paintOffset);
            }
